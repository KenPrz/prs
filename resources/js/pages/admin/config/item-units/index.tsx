...
export default function ItemUnitsIndex({
    ...
    itemUnits: Paginator<ItemUnitConfig>;
}) {
    const { url } = usePage();
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Item Units — Configuration" />
            <AdminConfigLayout>
                <ConfigPageHeader
                    title="Item Units"
                    description="Manage units of measurement for line items."
                    createHref="/admin/config/item-units/create"
                            ...
                            cell: (row) => (
                                <div className="flex items-center justify-end gap-1.5">
                                    <Button variant="outline" size="sm" asChild>
                                        <Link
                                            href={`/admin/config/item-units/${row.id}/edit`}
                                        >
                                            <Pencil className="h-4 w-4" />
                                        </Link>
                                    </Button>
                                    <ConfigDeleteDialog
                                        deleteUrl={`/admin/config/item-units/${row.id}`}
                                        label={`item unit "${row.code}"`}
                                    />
                                </div>
                            ),
